...
void ZenSonifier::sonifyPiece(Chess::Square const& square, Chess::Piece const& piece) {
    ...
    if (piece.type == Chess::Piece::Type::Pawn) {
        pieceInst = std::make_shared<CWavetableOscillator>(sine, FREQ::noteToFreq(mPawnNotes[square.file]), mGains[square.rank] / 0.4f, mSampleRate);
        ...
        pieceInst = std::make_shared<CWavetableOscillator>(sine, FREQ::noteToFreq(mAuxiliaryNotes[square.file]), mGains[square.rank], mSampleRate);
        pieceInst->setADSRParameters(1, 0, 1, 4);
    }
