...
void ZenSonifier::sonifyPiece(Chess::Square const& square, Chess::Piece const& piece) {
    ...
    if (piece.type == Chess::Piece::Type::Pawn) {
        ...
        auto inst1 = std::make_unique<CWavetableOscillator>(Waveform::sine, FREQ::noteToFreq(mPawnNotes[square.file]), 0.4, mSampleRate);
        ...
        auto inst = std::make_unique<CWavetableOscillator>(Waveform::sine, FREQ::noteToFreq(mRoyaltyNotes[square.file]), 0.5f, mSampleRate);
